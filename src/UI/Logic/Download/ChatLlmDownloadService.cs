...
public interface IChatLlmDownloadService
{
    ...
    Task DownloadModelQwen3ForcedAligner(string destinationFileName, IProgress<float>? progress, CancellationToken cancellationToken);
...
public class ChatLlmDownloadService : IChatLlmDownloadService
{
    ...
    private const string ModelsQwen3ForcedAligner = "https://modelscope.cn/models/judd2024/chatllm_quantized_qwen3/resolve/master/qwen3-focedaligner-0.6b.bin";
    ...
    public async Task DownloadModelQwen3ForcedAligner(string destinationFileName, IProgress<float>? progress, CancellationToken cancellationToken)
    {
        await DownloadHelper.DownloadFileAsync(_httpClient, ModelsQwen3AsrLargeUrl, destinationFileName, progress, cancellationToken);
    }
